...
def update_config_datasetmy(config_dict, outputdir, featuresdir):
    """Config for Dataset-1."""

    # Training
    config_dict['training']['df_train_path'] = '/code/BinFuncMatch/expriment/exp1/func_list/noinline_train_list.csv'
    config_dict['training']['features_train_path'] = '/dataset/full_dataset/stripped/noinline'

    # Validation
    config_dict['validation'] = dict(
        positive_path='/code/BinFuncMatch/expriment/exp1/pools_new/validation/pos.csv',
        negative_path='/code/BinFuncMatch/expriment/exp1/pools_new/validation/neg.csv',
        features_validation_path='/dataset/full_dataset/stripped/noinline'
    )

    # Testing
    fw_test_inputs = []
    fw_test_outputs = []
    test_path = '/dataset/full_dataset/stripped/inline'
    for root, _, files in os.walk(test_path):
        for file in files:
            if file.endswith('_gemini_pairs.csv'):
                if os.path.exists(os.path.join(root, file.replace('_gemini_pairs.csv', '_safe_embedding.pkl'))):
                    continue
                if not os.path.exists(os.path.join(root, file.replace('_gemini_pairs.csv', '_i2v_embed.json'))):
                    continue
                fw_test_inputs.append(os.path.join(root, file))
                fw_test_outputs.append(os.path.join(root, file.replace('.csv', '_sim.csv')))
    log.info(f'Remain {len(fw_test_inputs)} files to test')

    
    
    config_dict['testing'] = dict(
        full_tests_inputs= fw_test_inputs,
        full_tests_outputs= fw_test_outputs,
        features_testing_path= test_path
    )
...
def get_config(args):
    ...
    if args.dataset == 'one':
        ...
        update_config_datasetmy(config_dict, args.outputdir, args.featuresdir)
