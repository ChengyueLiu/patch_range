...
class PairFactoryInference(PairFactoryBase):
    ...
    def __init__(self, func_path, feat_path, batch_size, max_ins):
        ...
        p = func_path.replace('_gemini_pairs.csv', '_i2v_embed.json')
        self._fdict = {os.path.basename(p.replace('_i2v_embed.json', '.i64')):{}}
        if os.path.exists(p):
            with open(p) as gfd_in:
                ...
                self._fdict[os.path.basename(p.replace('_i2v_embed.json', '.i64'))] = new_d
    ...
    def _get_next_pair(self):
        ...
        while True:
            ...
            for _ in range(self._num_func_pairs):
                # Get the next row
                r = next(iterator)[1]
                if r['fva_1'] not in self._fdict[os.path.basename(r['idb_path_1'])] or r['fva_2'] not in self._fdict[os.path.basename(r['idb_path_2'])]:
                    r = next(iterator)[1]
                    self.not_found += 1
                    print(self.not_found, 'functions not found', r['idb_path_1'])
                    continue

                # Get the features for the left function
                f_l = self._fdict[os.path.basename(r['idb_path_1'])][r['fva_1']]["idx_list"]
                # ... and for the right one
                f_r = self._fdict[os.path.basename(r['idb_path_2'])][r['fva_2']]["idx_list"]
                yield ((f_l, f_r)), (r['idb_path_1'], r['fva_1'],
    ...
    def pairs(self):
        ...
        for _ in tqdm(range(self._num_batches),
            ...
            for _ in range(self._batch_size):
                try:
                    fpairs, pair_info = next(self._get_next_pair_it)
                except:
                    continue
