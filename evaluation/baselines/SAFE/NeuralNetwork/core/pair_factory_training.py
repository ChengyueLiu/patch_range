...
class PairFactoryTraining(PairFactoryBase):
    ...
    def _load_data(self, func_path, feat_path):
        ...
        for k in self._pairable_func_name_dict.keys():
            ...
            for v in self._pairable_func_name_dict[k]:
                if src_file != os.path.basename(self._df_func.iloc[v]['src_file']):
        ...
        for root, dirs, files in os.walk(feat_path):
            for file in files:
                if file.endswith('i2v_embed.json'):
                    ...
                    with open(p) as gfd_in:
                        ...
                        self._fdict[p.replace('_i2v_embed.json', '.i64')] = new_d
    ...
    def _select_random_function_pairs(self):
        ...
        while (1):
            ...
            fn1 = self._random.sample(self._pairable_func_name_list, k=1)[0]
            ...
            if len(func_poll_one) >= 2:
                ...
                break
        while True:
            fn2 = self._random.sample(self._func_name_list, k=1)[0]
            if fn1 == fn2:
                continue
            ...
            if len(func_poll_two) >= 1:
                ...
                if os.path.basename(self._df_func.iloc[idx3]['src_file']) == os.path.basename(self._df_func.iloc[idx1]['src_file']):
                    continue
