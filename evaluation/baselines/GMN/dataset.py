...
class GraphEditDistanceDataset(GraphSimilarityDataset):
    ...
    """
        ...
        with open(dataset_path, 'rb') as handle:
            self._dataset = pkl.load(handle)
            self._binary = None
            self._func = None
            self._graph = None

    def _get_next(self):
        if self._binary is None and self._func is None and self._graph is None:
            self._binary = iter(self._dataset)
            binary = next(self._binary, None)
            self._func = iter(self._dataset[binary])
            func = next(self._func, None)
            self._graph = iter(self._dataset[binary][func])
            graph = next(self._graph, None)
        else:
            graph = next(self._graph, None)
            if graph is None:
                func = next(self._func, None)
    ...
    def _to_graph(graph):
        g = graph
        mapping = {node: idx for idx, node in enumerate(g.nodes, start=0)}
    ...
    def _get_pos(self):
        while True:
            try:
                binary, func, graph1 = self._get_next()
                graph2 = random.choice(self._dataset[binary][func])
                if graph2 == graph1:
                    continue
                g1, g2 = graph1, graph2
                break
    ...
    def _get_neg(self):
        while True:
            try:
                binary1, func1, graph1 = self._get_next()
                binary2 = random.choice(list(self._dataset.keys()))
                func2 = random.choice(list(self._dataset[binary2].keys()))
                if binary1 == binary2 and func1 == func2:
                    continue
                graph2 = random.choice(self._dataset[binary2][func2])
                g1, g2 = graph1, graph2
                break
            except:
                pass
    ...
    def _get_triplet(self):
        ...
        while True:
            try:
                binary1, func1, graph1 = self._get_next()
                graph2 = random.choice(self._dataset[binary1][func1])
                if graph2 == graph1:
                    continue
                binary2 = random.choice(list(self._dataset.keys()))
                func2 = random.choice(list(self._dataset[binary2].keys()))
                if binary1 == binary2 and func1 == func2:
                    continue
                graph3 = random.choice(self._dataset[binary2][func2])
                g1, g2, g3 = graph1, graph2, graph3
                break
            except:
                pass
        return g1, g2, g3
    ...
    """
        ...
        return GraphData(
            ...
            # node_features=np.ones((n_total_nodes, 69), dtype=np.float32),
