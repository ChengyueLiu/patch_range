...
# Print configure
config = get_default_config(dataset_path='train_dataset.pkl')
    ...
    if (i_iter + 1) % config['training']['print_after'] == 0:
        ...
        if ((i_iter + 1) // config['training']['print_after'] %
            ...
            with torch.no_grad():
                ...
                if eval_metrics['pair_auc'] > best:
                    best = eval_metrics['pair_auc']
                    save_model(model, optimizer, best, path=config['cp_path'])
                info_str += ', ' + ', '.join(
