...
class DatasetBase(object):
    ...
    def traverse_file(self):
        print('Traversing', self.path, self.all_data, self.prefixfilter)
        for root, dirs, files in os.walk(self.path):
            for file in files:
                if file.endswith('jtrans_fea.pkl'):
                    bin_name = file.replace('jtrans_fea.pkl', '').split('-')[-1]
                    yield bin_name, file, os.path.join(root, file)
            # for dir in dirs:
            #     if self.all_data:
            #         for file in os.listdir(os.path.join(root, dir)):
            #             if not file.endswith('jtrans_fea.pkl'):
            #                 continue
