...
class Embedding:
	...
	def embed_a_binary(self, idafile, target_name=None):
		...
		func_list = pd.read_csv(idafile.replace('_palmtree_fea.pkl', '_func_list.csv'))
		fva_dict = {}
		...
		while True:
			...
			fvas.extend(fva_list)
		if len(not_found) > 0:
			print(f'Not found {len(not_found)} / {len(func_name_list)}')
