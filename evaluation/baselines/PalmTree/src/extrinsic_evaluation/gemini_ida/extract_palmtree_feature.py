...
IDA_SCRIPT = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'IDA_palmtree_feature.py')
...
if __name__ == '__main__':
    ...
    # unrar, openssl, zeromq, json, leveldb, libwebp, libtiff, PuTTy
    test_dirs = [
        '/dataset/full_dataset/stripped/inline/unrar',
        '/dataset/full_dataset/stripped/inline/openssl',
        '/dataset/full_dataset/stripped/inline/zeromq',
        '/dataset/full_dataset/stripped/inline/json',
        '/dataset/full_dataset/stripped/inline/leveldb',
        '/dataset/full_dataset/stripped/inline/libwebp',
        '/dataset/full_dataset/stripped/inline/libtiff',
        '/dataset/full_dataset/stripped/inline/PuTTy'
    ]
    ...
    if os.path.isdir(input_path):
        to_handle = []
        for input_path in test_dirs:
            for root, dirs, files in os.walk(input_path):
                for file in files:
                    if file.endswith('.i64') or file.endswith('.idb'):
                        if not file.startswith('x'):
                            continue
                        if not os.path.exists(os.path.join(root, file.replace('.i64', '_palmtree_fea.pkl'))):
        ...
        p = subprocess.Popen([IDA_PATH, '-A', '-Lida_export.log', f'-S{IDA_SCRIPT}', f'-Oidb:{input_path}', input_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        p.wait()
