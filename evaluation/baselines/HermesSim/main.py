...
def generate_output_dir(args, base_output_dir="outputs", custom_desc=None):
    feature_name = (basename(args.features_dir) +
                    "-") if args.features_dir is not None else ""
...
def iter_configs(args):
    ...
    for desc, params in tunning_params.items():
        ...
        # if args.features_dir is None or not isdir(args.features_dir):
        #     print("[!] Non existing features_dir: {}".format(args.features_dir))
        #     return
...
def model_test_single_binary(gnn_model, input_dir):
    log.info("Running model testing")
    gnn_model.model_test_single_binary(input_dir)
...
def main():
    ...
    parser.add_argument('-t', '--task', required=True, default='Test',
                        help='Train or Test')

    parser.add_argument("--features_dir", required=False, default=None,
                        help="Path to the lifted pcode raw")

    parser.add_argument("--train_list", required=False, default=None,
                        help="Path to the func list csv of the validation dataset,"
                        "required if task is Train.")
    parser.add_argument("--train_feat", required=False, default=None,
                        help="Path to the preprocessed pkl of the train dataset,"
                        "required if task is Train.")
    parser.add_argument("--validation_group", required=False, default=None,
                        help="Path to the func group csv of the validation dataset,"
                        "required if task is Train.")
    parser.add_argument("--validation_feat", required=False, default=None,
                        help="Path to the preprocessed pkl of the validation dataset,"
                        "required if task is Train.")
    parser.add_argument("--test_list", required=False, default=None,
                        help="Path to the func list csv of the test dataset,"
                        "required if task is Test.")
    parser.add_argument("--test_feat", required=False, default=None,
                        help="Path to the preprocessed pkl of the test dataset,"
                        "required if task is Test.")
    ...
    for config, desc, testing in iter_configs(args):
        ...
        if testing:
            ...
            model_test_single_binary(gnn_model, args.test_list)
        else:
