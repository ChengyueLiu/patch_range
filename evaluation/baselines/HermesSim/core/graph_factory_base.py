...
def pack_batch(graphs, features):
    from_idx = []
    to_idx = []
    node_features = []
    graph_idx = []
...
class GraphFactoryBase(object):
    ...
    def __init__(self, mode, feat_path, batch_size, used_subgraphs, edge_feature_dim) -> None:
        ...
        cache_path = cache_path.replace('.pkl', '')
        ...
        f_dict = {}
        for idb in self._fdict:
            f_dict[idb] = {}
            for fva in self._fdict[idb]:
                f_dict[idb][int(fva, 16)] = self._fdict[idb][fva]
        self._fdict = f_dict
