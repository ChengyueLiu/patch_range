...
1. **SAFE case**（96 个）：从程序分析找到的 VULN 点往前搜索，找到真正起点。平均距离 69 个版本。
2. **NoVuln case**（93 个）：add-only patch 或极少 meaningful deleted lines，完全靠 LLM 在所有 states 中定位。
3. **EARLY case**（38 个）：需要 LLM 验证是否确实是 GT 标注问题。
